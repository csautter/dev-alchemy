...
func RunHypervWindowsBuildOnWindows(config VirtualMachineConfig) error {
	...
	args := []string{"build", "-var", "iso_url=./vendor/windows/win11_25h2_english_amd64.iso", "-var", "cpus=" + getVmCpuCountString(config), packer_file}
	return RunBuildScript(config, "packer", args)
}

func RunVirtualBoxWindowsBuildOnWindows(config VirtualMachineConfig) error {
	packer_file := "build/packer/windows/windows11-on-windows-virtualbox.pkr.hcl"
