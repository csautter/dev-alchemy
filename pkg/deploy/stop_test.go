...
func TestSupportsStop(t *testing.T) {
	tests := []struct {
		...
		{
			name: "linux qemu windows supported",
			vm: alchemy_build.VirtualMachineConfig{
				OS:                   "windows11",
				Arch:                 "amd64",
				HostOs:               alchemy_build.HostOsLinux,
	...
	for _, tt := range tests {
		if got := SupportsStop(tt.vm); got != tt.want {
			t.Fatalf("%s: expected %v, got %v", tt.name, tt.want, got)
		}
	}
}

func TestRunStopReturnsHelpfulUnsupportedError(t *testing.T) {
	vm := alchemy_build.VirtualMachineConfig{
		OS:                   "windows11",
		Arch:                 "amd64",
		HostOs:               alchemy_build.HostOsWindows,
	...
	err := RunStop(vm)
	if err == nil {
		t.Fatal("expected unsupported stop to return an error")
	}
